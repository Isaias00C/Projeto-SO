...
public class ChildThreadGUI extends Thread {
    ...
    private static List<Float> posx = new ArrayList<>();
    private static List<Float> posy = new ArrayList<>();
...
boolean muitoProximo;
        do {
            muitoProximo = false;
            int side = RNG.nextInt(2);
            switch (side) {
                case 0:
                    cornerX = 0.05f + RNG.nextFloat() * (0.40f - 0.05f);
                    cornerY = 0.0f + RNG.nextFloat() * (0.75f - 0.0f);
                    break;
                case 1:
                    cornerX = 0.60f + RNG.nextFloat() * (0.95f - 0.60f);
                    cornerY = 0.0f + RNG.nextFloat() * (0.75f - 0.0f);
                    break;
            }

            // 2. Checagem por Pitágoras
            for (int i = 0; i < posx.size(); i++) {
                float dx = cornerX - posx.get(i);
                float dy = cornerY - posy.get(i);
                
                // d² = dx² + dy²
                double distanciaSqr = Math.pow(dx, 2) + Math.pow(dy, 2);
                double distanciaReal = Math.sqrt(distanciaSqr);


                if (distanciaReal < 0.20f) {
                    muitoProximo = true;
                    break;
                }
            }
        } while (muitoProximo); // Se cair aqui, o loop recomeça e sorteia novo X e Y
